...
class CubesEnv:
    ...
    def reset_cam(self,camDistance=1):
        self.cam_target_pos=[np.random.randint(self.gap,self._width-self.gap),
        np.random.randint(self.gap,self._length-self.gap),
        np.random.randint(self.gap,self._height-self.gap)]
    ...
    def reset(self):
        ...
        for i in range(10000):
            p.stepSimulation()
            self.my_setJointMotorControlMultiDof()  #IL FAUT PAS L OUBLIER DANS CETTE LIGNE
            time.sleep(1./240.)
        '''
    ...
    def reward(self):
        ...
        prob=toolbox.getRewardProb(self.nn,obs)
...
def start_env():
    ...
    ct,tt,st=CNNtrain(2,model,loader) #entrainement du model
